...
6. Add React wrappers for new icons in `packages/nimble-components/src/icon-base/tests/icons.react.tsx`.
7. Run `npm run build -w @ni/nimble-components` again. It should now succeed.
8. Preview the built files by running: `npm run storybook -w @ni/nimble-components`, and review the **Icons** story to confirm that your changes appear correctly. Inspect the icons in each **Severity** and ensure their color changes. Ensure the table is large enough to see all icons without nested scrollbars.
