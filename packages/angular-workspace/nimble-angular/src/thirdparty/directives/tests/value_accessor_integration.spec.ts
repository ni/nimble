/**
 * [Nimble]
 * Copied from https://github.com/angular/angular/blob/v21.2.19/packages/forms/test/value_accessor_integration_spec.ts
 ...
 * - Comment out tests for the multi-select
 * - Comment out tests for range input
 */
...
import {
 ...
 // [Nimble] additions
  ...
  CheckboxControlValueAccessor as AngularCheckboxControlValueAccessor,
  NumberValueAccessor as AngularNumberValueAccessor,
  RadioControlValueAccessor as AngularRadioControlValueAccessor,
...
const isNode = false;
// [Nimble] Copied from @angular/private/testing
...
@Directive({
    selector:
    'input:not([type])[formControlName],input[type=text][formControlName],textarea[formControlName],input:not([type])[formControl],input[type=text][formControl],textarea[formControl],input:not([type])[ngModel],input[type=text][ngModel],textarea[ngModel],[ngDefaultControl]',
    host: {
...
// [Nimble] Create array of Angular Directives to not use in these tests
const angularDirectivesToOverwrite = [
  AngularCheckboxControlValueAccessor,
  AngularDefaultValueAccessor,
  AngularNumberValueAccessor,
  AngularRadioControlValueAccessor,
  AngularSelectControlValueAccessor,
...
describe('value accessors', () => {
  ...
  function initTest<T>(component: Type<T>, ...directives: Type<any>[]): ComponentFixture<T> {
    ...
    angularDirectivesToOverwrite.forEach(x => {
      TestBed.overrideDirective(x, { set: { selector: 'matches-nothing' } });
    });
  ...
  it('should ignore the change event for <input type=text>', () => {
    ...
    // [Nimble] Add a known empty expect to suppress error
    expect().nothing();
  });
  ...
  /* [Nimble] Don't run tests that require multi-selects
  ...
  */
  ...
  /* [Nimble] Skip tests for range input because those directives are not duplicated within nimble
  ...
  */
  ...
  describe('custom value accessors', () => {
    describe('in reactive forms', () => {
      it('should support basic functionality', () => {
        // [Nimble] Remove TestDefaultValueAccessor because there can only be one custom accessor on each control
        TestBed.overrideDirective(
          TestDefaultValueAccessor, { set: { selector: 'matches-nothing' }}
        );
