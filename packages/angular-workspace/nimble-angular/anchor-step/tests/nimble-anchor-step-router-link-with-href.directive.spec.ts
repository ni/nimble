...
import { processUpdates } from '../../src/testing/async-helpers';
...
describe('Nimble anchor step RouterLinkWithHrefDirective', () => {
    @Component({
        template: `
            <nimble-anchor-step #anchorStep nimbleRouterLink="page1" [queryParams]="{param1: true}" [state]="{stateProperty: 123}">
                Anchor text
            </nimble-anchor-step>
         `,
        standalone: false
    ...
    beforeEach(() => {
        ...
        anchorStep = testHostComponent.anchorStep.nativeElement;
        processUpdates();
        innerAnchor = anchorStep!.shadowRoot!.querySelector('a')!;
