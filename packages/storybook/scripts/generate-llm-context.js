...
class StoryFileParser {
    ...
    inferType(config) {
        ...
        if (config.control && config.control.type === 'boolean') return 'boolean';
        if (config.control && config.control.type === 'number') return 'number';
        return 'string';
    }
...
Attributes:
${api.attributes.filter(attr => attr.category === 'attributes').map(attr => 
    `- ${attr.name}: ${attr.type}${attr.defaultValue ? ` = ${JSON.stringify(attr.defaultValue)}` : ''}
