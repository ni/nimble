...
const severityStates = [
    ['Default', BannerSeverityLit.default],
    ['Error', BannerSeverityLit.error],
    ['Warning', BannerSeverityLit.warning],
    ['Information', BannerSeverityLit.information]
] as const;
type SeverityState = (typeof severityStates)[number];

const metadata: Meta = {
    title: 'Tests/Banner Lit',
    parameters: {
        ...sharedMatrixParameters()
    }
};
...
export const bannerLitThemeMatrix: StoryFn = createMatrixThemeStory(
...
export const hiddenBannerLit: StoryFn = createStory(
    hiddenWrapper(
        html`<${bannerLitTag} hidden>
