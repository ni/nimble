...
The table will reserve space for expand/collapse row buttons and the collapse all button in the header when there are no collapsible rows if the table is configured to have either:

1. At least one column that supports grouping (i.e. `groupingDisabled` is false), or
2. Has hierarchy (i.e. `parentIdFieldName` is defined)

To prevent this space from being reserved and make the table data full bleed, set `groupingDisabled` to `true` on all groupable columns and do not set `parentIdFieldName` on the table.
