...
selected using the selection checkboxes. Additionally, clicking on rows has the following behavior:

-   Clicking on a row selects just that row
-   `CTRL` clicking on a row toggles the selection of that row while maintaining the selection of other rows
-   Selecting a row followed by `SHIFT` clicking on another row selects the range of rows between the two rows
-   Selecting a row followed by `SHIFT-CTRL` clicking on a different row adds the range of rows to the existing selection
