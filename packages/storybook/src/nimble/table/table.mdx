...
by styling the table's height with the <code>{tableFitRowsHeight.name}</code> token.

The table has a default maximum height that will render approximately 40 rows to avoid performance problems. Use caution when overriding the maximum height of the table because this may lead to performance issues.
