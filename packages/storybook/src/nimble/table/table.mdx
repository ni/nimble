...
When the table has the `selectionMode` property set to `multiple`, multiple rows can be selected using the selection checkboxes. Additionally, multiple rows can be selected by pressing the `CTRL` key while clicking each row, and a range of rows can be selected by pressing the `SHIFT` key while clicking on one row and then another.
