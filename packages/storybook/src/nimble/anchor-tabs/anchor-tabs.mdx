...
If you want the tabs to switch between different tab panels hosted on the same page, use the <Tag name={tabTag}/> instead.
...
Use the <Tag name={anchorTabsTag} /> child element to add new tabs to the <Tag name={anchorTabTag} />.
...
on each <Tag name={anchorTabTag} /> element. In those cases, it is recommended to also set `replaceUrl="true"` so that switching
