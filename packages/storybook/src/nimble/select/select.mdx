    ...
    - Retrieve the filter text from the event and populate the DOM with the set
      ...
      reduce unnecessary requests, consider debouncing the filter input.
        - If the filter text is empty, the select should display all initial
          options in the dropdown in addition to the current selected option
          if it is not one of the initial options.
        - If the current selected value isn't part of the filtered options,
          consider updating the hidden state of the selected option (as it 
          should still be included in the options in DOM).
