...
1. In the event handler, retrieve the filter text from the event and populate
   the DOM with the set of options that match the filter text by your own
   logic. - It is important that one of the options in the DOM is the currently
   selected option.
