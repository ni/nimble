...
The `Select` component can be configured to display a placeholder when no option
is selected. To do this, configure one option to have its `selected`,
`disabled`, and `hidden` attributes set to `true`.
