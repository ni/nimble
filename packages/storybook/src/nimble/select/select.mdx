...
of options. The select offers the `filter-input` event to allow clients to
listen for changes to the filter input. When accompanied with the setting `filter-mode`
