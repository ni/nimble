...
import { NimbleSpinner } from '@ni/nimble-react/dist/esm/spinner/spinner.react';
...
-   Avoid having multiple values that map to the same text value. This can lead
    to confusion when grouping and sorting, as it would result in multiple
    groups with the same text label representing different values or the same
    text label showing up multiple times with a different sort order.
-   Don't mix and match icon-only, and icon and text mappings in the same
    column. Choose whether to enable the `text-hidden` attribute for all
    mappings or none.
