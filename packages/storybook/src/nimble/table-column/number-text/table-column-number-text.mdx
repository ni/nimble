...
-   As input, the pipe optionally takes a configuration object as a parameter. The object may include any of the following properties:
    ...
    -   `unitScale` - A `UnitScale` object indicating units to display. Possible values are `byteUnitScale`, `byte1024UnitScale`, `celsiusUnitScale`, `fahrenheitUnitScale`, and `voltUnitScale` which are exported from `@ni/nimble-angular/pipes`.
