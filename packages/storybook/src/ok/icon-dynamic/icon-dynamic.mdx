...
Register icons before use. Names must be lowercase letters and at least two
characters long. The <code>registerIconDynamic</code> call throws if the name
is invalid. For example:

```ts
customElements
    .get('ok-icon-dynamic')
    .registerIconDynamic('awesome', 'data:image/png;base64,...');
```
