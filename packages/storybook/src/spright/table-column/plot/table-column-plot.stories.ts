...
import { tableColumnPlotTag } from '../../../../../spright-components/src/table-column/plot-column';
import {
    NumberTextAlignment,
    NumberTextFormat
...
const metadata: Meta<SharedTableArgs> = {
    title: 'Spright/Table Column: Plot',
...
export const numberTextColumn: StoryObj<NumberTextColumnTableArgs> = {
    ...
    render: createUserSelectedThemeStory(html<NumberTextColumnTableArgs>`
        ...
        >
            ...
            <${tableColumnPlotTag} field-name="age" plot-enabled-field-name="plot" format="${x => NumberTextFormat[x.format]}" alignment="${x => NumberTextAlignment[x.alignment]}" decimal-digits="${x => x.decimalDigits}" decimal-maximum-digits="${x => x.decimalMaximumDigits}">
                Age
            </${tableColumnPlotTag}>
            <${tableColumnPlotTag} field-name="favoriteNumber" plot-enabled-field-name="plot1" format="${x => NumberTextFormat[x.format]}" alignment="${x => NumberTextAlignment[x.alignment]}" decimal-digits="${x => x.decimalDigits}" decimal-maximum-digits="${x => x.decimalMaximumDigits}" placeholder="${x => x.placeholder}">
                Favorite Number
            </${tableColumnPlotTag}>
            <${tableColumnPlotTag} field-name="measurement" plot-enabled-field-name="plot2" format="${x => NumberTextFormat[x.format]}" alignment="${x => NumberTextAlignment[x.alignment]}" decimal-digits="${x => x.decimalDigits}" decimal-maximum-digits="${x => x.decimalMaximumDigits}" placeholder="${x => x.placeholder}">
                Measurement
