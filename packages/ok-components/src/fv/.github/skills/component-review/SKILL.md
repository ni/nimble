...
- For components with dropdowns, menus, or popups: verify `nimble-anchored-region` with `fixed-placement` is used (not absolute/relative positioning). Verify the anchor is wired via `@observable` properties (`region`, `field`) + `regionChanged`/`fieldChanged` lifecycle methods — NOT manual ref-capture callback methods. Treat use of `captureAnchoredRegionRef`-style methods instead of the observable property pattern as review feedback.
...
- Verify story boilerplate elements (warning banners, injected styles) use `class="code-hide"` so they are stripped from the displayed code snippet.
