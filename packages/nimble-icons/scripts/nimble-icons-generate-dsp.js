...
// Run with `node nimble-icons-generate-dsp.js` or `npm run generate-dsp`
...
icons = files.map(file => file.split('.')[0]);
...
const data = prettier.format(JSON.stringify(dsp), { semi: false, parser: 'json-stringify' });
fs.writeFileSync('./data/components.json', data);