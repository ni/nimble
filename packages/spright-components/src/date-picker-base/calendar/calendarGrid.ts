...
const getWeekdays = (locale: DatePickerLocale): Weekday[] => {
    const firstDayOfWeek = locale.firstDayOfWeek;
    const localeWeekdayToNative = (day: number): number => (day + firstDayOfWeek) % 7;
...
const gridDate = (date: Date, isOutsideMonth2: boolean): CalendarGridDate => ({
...
function isOutsideMonth(dayIndexInMonth: number, daysInMonth: number): boolean {
...
export const buildCalendarGrid = (
    ...
    const getShiftedDay = (date: Date): number => (date.getDay() - firstDayOfWeek + 7) % 7;
