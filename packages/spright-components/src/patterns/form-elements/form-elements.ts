...
import { attr, html, observable, slotted, ViewTemplate, when } from '@microsoft/fast-element';
...
export class FormElementHelperText {
     ...
     */
    @observable
    public _helperTextSlottedContent?: HTMLElement[];
}
...
export class FormElementCharCount {
    @attr({ attribute: 'char-count', mode: 'boolean' })
...
>(constructor: T) {
        ...
        if (currentPrototype.validate && !parentPrototype.validate) {
            currentPrototype.validate = function (anchor?: HTMLElement) {
                if (this.proxy instanceof HTMLElement && this.elementInternals) {
                    ...
                    const controlIsInvalidDueToMinOrMaxLength = this.control
    ...
    class Decorated extends constructor {
         ...
         */
        @observable public errorValidationMessage = '';
...
const feedback: { [key: string]: FeedbackConfig } = {
    helper: {
...
const isFeedbackAvailable = (config: FeedbackConfig, x: SomeFormElement): boolean => Boolean(
    x[config.messageProperty]
            || (config.slot && x[config.slot.slottedContentProperty]?.length)
);

export function getFeedbackTemplate(context: ElementDefinitionContext): ViewTemplate {
    return html<SomeFormElement>`
        ${getFeedbackTypeTemplate(
        context,
        feedback.helper!,
        x => isFeedbackAvailable(feedback.helper!, x)
        ...
        context,
        feedback.error!,
        x => isFeedbackAvailable(feedback.error!, x)
                && !isFeedbackAvailable(feedback.success!, x)
    )}
        ${getFeedbackTypeTemplate(context, feedback.success!, x => isFeedbackAvailable(feedback.success!, x))}
    `;
}
...
function getFeedbackTypeTemplate(
    ...
    >
        ${when(
        x => shouldShow(x) && config.iconType,
        html`<span>icon: "${config.iconType!}" here</span>`
    )}
...
>(constructor: T) {
    ...
    class Decorated extends constructor {
        ...
        #forceCustomError(errorMessage: string): void {
        ...
        #clearCustomErrorAndRevalidate(): void {
