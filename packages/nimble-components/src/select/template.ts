...
> = (context, definition) => html<Select>`
    <template
        ...
        aria-expanded="${x => (x.readOnly ? null : x.ariaExpanded)}"
        aria-haspopup="${x => ((x.collapsible && !x.readOnly) ? 'listbox' : null)}"
        aria-label="${x => x.labelContent}"
        aria-multiselectable="${x => x.ariaMultiSelectable}"
        aria-required="${x => x.requiredVisible}"
        aria-readonly="${x => (x.readOnly ? 'true' : null)}"
        ?open="${x => x.open}"
        role="${x => (x.readOnly ? 'textbox' : 'combobox')}"
        ...
        ${when(x => x.collapsible, html<Select>`
            ...
            >
                ...
                <slot name="button-container">
                    ...
                    ${when(x => !(x.disabled || x.readOnly) && x.clearable && !x.displayPlaceholder && x.selectedIndex >= 0, html<Select>`
                        <${buttonTag} 
                            class="clear-button"
                            tabindex="-1"
                            part="clear-button"
