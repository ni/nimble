...
describe('Select', () => {
    ...
    describe('Default selected option', () => {
        ...
        test.each(defaultOptionTestCases)('$name', async (value: any) => {
            const { element, connect, disconnect } = await setup(
                undefined,
                false,
                value.option1State,
        ...
        describe('with 500 options', () => {
            ...
            it('should scroll the selected option into view when opened', async () => {
                ...
                expect(element.scrollableRegion.scrollTop).toBeGreaterThan(
                    8000
                );
        ...
        describe('opening and closing dropdown', () => {
            ...
            describe.each(filterModeTestData)(
                'with filterMode = $name',
                (value: any) => {
                    beforeEach(() => {
        ...
        describe('filtering', () => {
            ...
            describe('with filterMode = standard', () => {
                ...
                it('filter input placeholder gets text from design token', async () => {
                    ...
                    expect(filterInput?.getAttribute('placeholder')).toBe(
                        'foo'
                    );
                });
                ...
                it('filter input "aria-controls" and "aria-activedescendant" attributes are set to element state', async () => {
                    ...
                    expect(
                        filterInput?.getAttribute('aria-activedescendant')
                    ).toBe(element.ariaActiveDescendant);
                });

                it('pressing arrow keys selects next visible item when option between current and next is filtered out', async () => {
                    const newOptions = element.options.map(
                        (o) => o as ListOption
                    );
        ...
        describe('clearable', () => {
            ...
            describe('with placeholder', () => {
                ...
                test.each(removePlaceholderData)(
                    '$name',
                    async (value: any) => {
                        value.updatePlaceholder(
                            element.options[0]! as ListOption
                        );
                        await waitForUpdatesAsync();
                        expect(pageObject.isClearButtonVisible()).toBe(true);
                        pageObject.clickClearButton();
        ...
        describe('groups', () => {
            ...
            it('group that is hidden when slotted results in hidden options', async () => {
                ...
                expect(pageObject.isOptionVisible(hiddenOptionIndex)).toBe(
                    false
                );
            });
            ...
            it('hiding option between groups results in top separator being hidden of group after option', async () => {
                ...
                await clickAndWaitForOpen(element);
                const groupAfterInsertedOption = pageObject.getGroup(1);
                expect(groupAfterInsertedOption.topSeparatorVisible).toBe(
                    false
                );
            });
            ...
            it('placeholder can be defined in a group', async () => {
                const group = pageObject.getGroup(0);
                const placeholder = new ListOption(
                    'Placeholder',
                    'placeholder'
                );
            ...
            describe('filtering', () => {
                ...
                test.each(groupFilterTestData)('$name', async (value: any) => {
                    await connect();
                    await waitForUpdatesAsync();
                ...
                it('filter match on group label shows all options in matched group, as well as options in other groups that match filter', async () => {
                    ...
                    expect(visibleElements[3]!.textContent?.trim()).toBe(
                        'Three'
                    );
                    expect(visibleElements[4]!.textContent?.trim()).toBe(
                        'Four'
                    );
