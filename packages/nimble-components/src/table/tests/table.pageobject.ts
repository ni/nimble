...
export class TablePageObject<T extends ITableData> {
    ...
    public getRenderedRowCount(): number {
        return this.tableElement.shadowRoot!.querySelectorAll('.table-row').length || 0;
    }
    ...
    public getCellContent(rowIndex: number, columnIndex: number): string {
        ...
        if (rowIndex >= rows.length) {
            throw new Error('Attempting to index passed the total number of rendered rows');
        }

        const row = rows.item(rowIndex);
        const cells = row.querySelectorAll('.table-cell');
        if (columnIndex >= cells.length) {
            throw new Error('Attempting to index passed the totla number of rendered columns');
        }
