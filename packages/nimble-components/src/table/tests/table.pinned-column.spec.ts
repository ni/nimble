...
describe('Table pinned columns', () => {
	...
	it('changing width-mode to icon-size directly corresponds to resizingDisabled', async () => {
		await connect();
		await waitForUpdatesAsync();

		expect(column1.columnInternals.resizingDisabled).toBeFalse();

		column1.widthMode = TableColumnMappingWidthMode.iconSize;
		await waitForUpdatesAsync();

		expect(column1.columnInternals.resizingDisabled).toBeTrue();

		column1.widthMode = TableColumnMappingWidthMode.default;
		await waitForUpdatesAsync();
