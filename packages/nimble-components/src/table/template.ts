                            ...
                            ${repeat(x => x.virtualizer.visibleItems, html<VirtualItem, Table>`
                                ${when((x, c) => (c.parent as Table).tableData[x.index]?.isGrouped, html<VirtualItem, Table>`
                                    <${tableGroupRowTag}
                                        ...
                                        @group-expand-toggle="${(x, c) => c.parent.handleRowExpanded(x.index, c.event)}"
                                    >
                                    </${tableGroupRowTag}>
                                `)}
                                ${when((x, c) => !(c.parent as Table).tableData[x.index]?.isGrouped, html<VirtualItem, Table>`
                                    <${tableRowTag}
                                        ...
                                        :isParentRow="${(x, c) => c.parent.tableData[x.index]?.record.parentId === undefined}"
                                        :nestingLevel="${(x, c) => c.parent.tableData[x.index]?.nestingLevel}"
                                        ...
                                        @row-expand-toggle="${(x, c) => c.parent.handleRowExpanded(x.index, c.event)}"
                                        :dataIndex="${x => x.index}"
                                    >
