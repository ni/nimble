...
2.  Your `UnitScale` class goes under `src/unit/<name>/models`. You must determine whether your unit is among [those supported by `Intl.NumberFormat`](https://tc39.es/ecma402/#table-sanctioned-single-unit-identifiers).
    -   If it is, your new class should extend `UnitScale`, and you must implement the method `getSupportedScaledUnits()`. This should return an array of `IntlNumberFormatScaledUnit` instances. See `src/unit/byte/models/byte-unit-scale.ts` for an example.
