...
-   [#867 Interactive grouping of data rows](https://github.com/ni/nimble/issues/867)
...
>(base: TBase) {
    abstract class GroupableColumn extends base {
        ...
        /**
         * The custom element to use for rendering group header values.
...
Rendering a row group has concerns beyond just rendering the grouping value. It also has a button for controlling the expand/collapse state of a row group (indented for sub-groups) as well as potentially rendering auxiliary information such as the total number of items within a group. We can provide a new `TableRowGroup` element to manage all of these:

```
export interface TableRowGroupState {
...
export class TableRowGroup extends FoundationElement {
    @observable
    public rowGroupState: TableRowGroupState;

    public onExpandRowToggle(): void {
        const detail: TableRowGroupExpandToggleEventDetail = {
        };
...
const tableRowGroupElement = TableRowGroup.compose({
...
DesignSystem.getOrCreate().withPrefix('nimble').register(tableRowGroupElement());
```

Grouped rows are provided alongside ungrouped rows through the TanStack function `getRowModel().rows`, which is already used for retrieving the rows to render. So to render the `TableRowGroups`, the template can make a simple change similar to this:

Table template.ts
```
    ...
    ${repeat(x => x.virtualizer.visibleItems, html<VirtualItem, Table>`
        ${when((x, c) => (c.parent as Table).getIsRowGrouped(x.index), html<VirtualItem, Table>`
            <${DesignSystem.tagFor(TableRowGroup)}
                :rowGroupState="${(x, c) => c.parent.getRowGroupState(x.index)}"
