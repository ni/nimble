...
(() => {
    const themeProvider = document.querySelector('nimble-theme-provider');
    const componentName = queryParams.component;

    performance.mark(componentName);
    for (let i = 0; i < 100; i++) {
        themeProvider.append(document.createElement(componentName));
    }
    performance.measure('createElement', componentName);
})();