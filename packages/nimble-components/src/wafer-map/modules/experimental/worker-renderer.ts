...
export class WorkerRenderer {
    ...
    private async setupWorker(): Promise<void> {
        ...
        await this.wafermap.worker.setMaxCharacters(
            this.wafermap.maxCharacters
        );
        await this.wafermap.worker.setDieLabelsSuffix(
            this.wafermap.dieLabelsSuffix
        );
        if (this.wafermap.dataManager instanceof DataManager) {
            await this.wafermap.worker.setColors(
                this.wafermap.dataManager.colorScale.colors
            );
            await this.wafermap.worker.setColorValues(
