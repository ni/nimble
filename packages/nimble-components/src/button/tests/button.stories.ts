...
const metadata: Meta<ButtonArgs> = {
    ...
    // prettier-ignore
    render: ({ label, appearance, disabled, icon }: ButtonArgs): string => `
        <nimble-button ${disabled ? 'disabled' : ''} appearance="${appearance}">
            ${icon ? `<div slot="icon">${controlsSearch16X16.data}</div>` : ''}${label}
        </nimble-button>
`,
    args: {
        label: 'Ghost Button',
