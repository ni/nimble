...
describe('RichTextEditorMentionListbox', () => {
    ...
    describe('Dynamically update mention popup items based on configuration changes', () => {
        const secondaryClickTests = Array.from({ length: 20 }).map((_, index) => {
            return {
                name: `Test ${index}`
            };
        });
        ...
        parameterizeSpec(secondaryClickTests, ({ name }) => {
            it('should close mention popup when removing configuration element ' + `${name}`, async () => {
                const { userMentionElement } = await appendUserMentionConfiguration(
                    element,
                    [
                        { key: 'user:1', displayName: 'username1' },
            ...
            it('should update mention popup list when removing mapping element' + `${name}`, async () => {
                const { mappingElements, userMentionElement } = await appendUserMentionConfiguration(element, [
                    { key: 'user:1', displayName: 'username1' },
                    { key: 'user:2', displayName: 'username2' }
                ]);
            ...
            it('should update mention popup list when mapping elements get replaced' + `${name}`, async () => {
                const { userMentionElement } = await appendUserMentionConfiguration(
                    element,
                    [
                        { key: 'user:1', displayName: 'username1' },
            ...
            it('should close mention popup when updating to invalid `pattern`' + `${name}`, async () => {
                const { userMentionElement } = await appendUserMentionConfiguration(
                    element,
                    [
                        { key: 'user:1', displayName: 'username1' },
            ...
            it('should update mention popup list when updating `display-name`' + `${name}`, async () => {
                const { mappingElements } = await appendUserMentionConfiguration(
                    element,
                    [
                        { key: 'user:1', displayName: 'username1' },
            ...
            it('should update mention popup list when updating valid `key`' + `${name}`, async () => {
                const { mappingElements } = await appendUserMentionConfiguration(
                    element,
            ...
            it('should close mention popup when updating to invalid `key`' + `${name}`, async () => {
                const { mappingElements } = await appendUserMentionConfiguration(
                    element,
                    [
                        { key: 'user:1', displayName: 'username1' },
            ...
            it('should retain mention popup list position in the same cursor position when configuration got updated' + `${name}`, async () => {
                const { mappingElements } = await appendUserMentionConfiguration(
                    element,
                    [
                        { key: 'user:1', displayName: 'username1' },
            ...
            it(`should show mention popup for multiple mention configuration elements ${name}`, async () => {
                await appendUserMentionConfiguration(element, [
                    { key: 'user:1', displayName: 'username1' },
                    { key: 'user:2', displayName: 'username2' }
                ]);
            ...
            it('mention listbox should be closed when cursor position is moved to start and configuration dynamically changes' + `${name}`, async () => {
                const { mappingElements } = await appendUserMentionConfiguration(
                    element,
