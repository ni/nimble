...
1. The `UI Tests` status check is designed to highlight any visual changes included in the changeset. The developer (that's you!) should review the `UI Tests` status check in Chromatic, and if all changes are intentional or expected, mark the components as **approved**. Please wait until you're nearly ready to submit to do this; if you approve a snapshot and then the appearance changes as a result of PR feedback, the diffs can become confusing.
